...
Rectangle {
    ...
    // ============================================================
    // Background — 视频/图片 双模式背景 (toggleable video/image)
    // ============================================================
    // Pure black fallback layer (纯黑底垫)
    Rectangle {
        anchors.fill: parent
        color: "#000000"
        z: -1000
    ...
    // Video player — only active when useImage === false
    // 视频播放器，仅在图片模式关闭时运行
    MediaPlayer { 
        id: player
        source: "output.mp4"
    ...
    // Static image — shown when useImage === true
    // 静态图片，仅在图片模式时可见（替换背景中的图片文件名即可）
    Image {
        id: bgImage
        source: "background.jpg"   // ← change this to your image file
        anchors.fill: parent
